...
function Configuration() {
  ...
  const debouncedResults = useDebounce(results, { wait: 300 });
  ...
  const searchInProgress = results && results !== debouncedResults;
  const haveResults = !searchInProgress && debouncedResults && debouncedResults.length > 0;
        ...
        {isConfTab
          ...
          : (
            <>
              ...
              {searchInProgress && (
                <div role="status">
                  <svg aria-hidden="true" className="mr-2 w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-indigo-500" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <>
                  <div>
                    <p className="text-sm text-gray-700">
                      Showing
                      {' '}
                      <span className="font-medium">{debouncedResults.length}</span>
                      {' '}
                      results
                    </p>
                  </div>
                  <ReactJson
                    src={debouncedResults}
                    name={false}
                    onAdd={false}
                    onEdit={false}
                    onDelete={false}
